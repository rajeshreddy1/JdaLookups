...
public class Usp_Buyer_Search  extends StoredProcedure {

	@SuppressWarnings("rawtypes")
	public Usp_Buyer_Search(JdbcTemplate jdbcTemplate) {
		
		super(jdbcTemplate, "Usp_Buyer_Search");
		
		RowMapper rowMapper = new BuyerRowMapper();
		
		declareParameter(new SqlReturnResultSet("RESULT_LIST",rowMapper));
		
		declareParameter(new SqlParameter("p_buyer_Number" , Types.CHAR));      
		declareParameter(new SqlParameter("p_buyer_Name" , Types.VARCHAR));
	...
	public Map getResults(String buyerNumber, String buyerName, String sortFields, String existenceColumn, String appendToWhereClause, int start, int limit, String server) {
		
		Map inParms = new HashMap();

		inParms.put("p_buyer_Number"         , buyerNumber);
		inParms.put("p_buyer_Name"           , buyerName);
