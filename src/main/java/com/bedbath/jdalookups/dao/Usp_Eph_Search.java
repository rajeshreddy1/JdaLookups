...
public class Usp_Eph_Search  extends StoredProcedure {

	public Usp_Eph_Search(JdbcTemplate jdbcTemplate) {
		
		super(jdbcTemplate, "Usp_Eph_Search");
		
		declareParameter(new SqlReturnResultSet("RESULT_LIST", new EphSearchResultSetExtractor()));
		
		declareParameter(new SqlParameter("P_NODE_NM", Types.VARCHAR));
	...
	public Map<String, ? extends Object> getResults(String type, String description, String server, int start, int limit) {
		...
		inParms.put("P_NODE_NM", description);
